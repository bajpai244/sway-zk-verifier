...
function readSwayType(path2) {
  ...
  const swayEntryPath = join(path2, "src", entryFile);
...
function init(program) {
  ...
  if (options.workspace) {
    absoluteWorkspace = resolve3(path2, options.workspace);
    workspace = `./${relative(path2, absoluteWorkspace)}`;
  }
  const absoluteOutput = resolve3(path2, options.output);
  const output = `./${relative(path2, absoluteOutput)}`;
  const [contracts, scripts, predicates] = ["contracts", "scripts", "predicates"].map(
    (optionName) => {
      ...
      const expanded = globSync3(pathOrGlob, { cwd: path2 });
      const relatives = expanded.map((e) => relative(path2, e));
      return relatives;
    }
  );
  ...
  if (noneIsInformed) {
    ...
    const fuelsConfigPath = join3(path2, "fuels.config.ts");
...
var FUELS_VERSION_CACHE_FILE = path.join(__dirname, "FUELS_VERSION");
