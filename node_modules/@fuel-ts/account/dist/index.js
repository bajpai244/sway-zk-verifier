...
__export(src_exports, {
  ...
  getBytecodeConfigurableOffset: () => getBytecodeConfigurableOffset,
  getBytecodeDataOffset: () => getBytecodeDataOffset,
  getBytecodeId: () => getBytecodeId,
  ...
  getLegacyBlobId: () => getLegacyBlobId,
  getMaxGas: () => getMaxGas,
  getMinGas: () => getMinGas,
...
var BaseTransactionRequest = class {
  ...
  updateWitnessByOwner(address, signature) {
    const ownerAddress = new import_address.Address(address);
  ...
  addChangeOutput(to, assetId) {
    const changeOutput = this.getChangeOutputs().find(
      (output) => (0, import_utils10.hexlify)(output.assetId) === assetId
    );
    if (!changeOutput) {
      this.pushOutput({
  ...
  fundWithFakeUtxos(quantities, baseAssetId, resourcesOwner) {
    ...
    const updateAssetInput = (assetId, quantity) => {
      ...
      if (assetInput && "assetId" in assetInput) {
        assetInput.id = (0, import_utils10.hexlify)((0, import_crypto.randomBytes)(import_abi_coder2.UTXO_ID_LEN));
        assetInput.amount = usedQuantity;
      } else {
        this.addResources([
          {
            id: (0, import_utils10.hexlify)((0, import_crypto.randomBytes)(import_abi_coder2.UTXO_ID_LEN)),
            amount: usedQuantity,
            assetId,
  ...
  updatePredicateGasUsed(inputs) {
    ...
    this.inputs.filter(isRequestInputResource).forEach((i) => {
      ...
      const correspondingInput = inputsToExtractGasUsed.find(
        (x) => isRequestInputResourceFromOwner(x, new import_address.Address(String(owner)))
      );
...
var CreateTransactionRequest = class extends BaseTransactionRequest {
  ...
  constructor({ bytecodeWitnessIndex, salt, storageSlots, ...rest }) {
    ...
    this.salt = (0, import_utils15.hexlify)(salt ?? import_configs8.ZeroBytes32);
  ...
  toTransaction() {
    ...
    return {
      ...
      salt: this.salt ? (0, import_utils15.hexlify)(this.salt) : import_configs8.ZeroBytes32,
      storageSlots
    };
  }
  /**
  ...
  metadataGas(gasCosts) {
    return calculateMetadataGasForTxCreate({
      contractBytesSize: (0, import_math10.bn)((0, import_utils15.arrayify)(this.witnesses[this.bytecodeWitnessIndex] || "0x").length),
      gasCosts,
...
var ScriptTransactionRequest = class extends BaseTransactionRequest {
  ...
  constructor({ script, scriptData, gasLimit, ...rest } = {}) {
    ...
    this.script = (0, import_utils17.arrayify)(script ?? returnZeroScript.bytes);
    this.scriptData = (0, import_utils17.arrayify)(scriptData ?? returnZeroScript.encodeScriptData());
    this.abis = rest.abis;
  }
  /**
  ...
  toTransaction() {
    ...
    return {
      ...
      script: (0, import_utils17.hexlify)(script),
      scriptData: (0, import_utils17.hexlify)(scriptData)
    };
  }
  /**
...
var UploadTransactionRequest = class extends BaseTransactionRequest {
  ...
  toTransaction() {
    ...
    return {
      ...
      subsectionIndex,
      subsectionsNumber,
      root: (0, import_utils20.hexlify)(root),
      proofSet: proofSet.map(import_utils20.hexlify),
  ...
  metadataGas(gasCosts) {
    return calculateMetadataGasForTxUpload({
      gasCosts,
      txBytesSize: this.byteSize(),
      subsectionSize: (0, import_utils20.arrayify)(this.witnesses[this.witnessIndex]).length,
  ...
  calculateMinGas(chainInfo) {
    ...
    return calculateMinGasForTxUpload({
      ...
      subsectionSize: (0, import_utils20.arrayify)(bytecode).length
    });
  }
};
...
function assembleTransactionSummary(params) {
  ...
  const rawPayload = (0, import_utils24.hexlify)(transactionBytes);
  ...
  let date;
  if (time) {
    date = import_utils24.DateTime.fromTai64(time);
  }
...
function mapGqlOutputsToTxOutputs(outputs) {
  return outputs.map((o) => {
    ...
    switch (obj.type) {
      ...
      default:
        return (0, import_utils26.assertUnreachable)(obj);
    }
  });
}
var TransactionResponse = class {
  ...
  async getTransaction() {
    ...
    return {
      ...
      bytes: (0, import_utils26.arrayify)(gqlTransaction.rawPayload)
    };
  }
  getReceipts() {
...
function autoRetryFetch(fetchFn, options, retryAttemptNum = 0) {
  ...
  return async (...args) => {
    try {
      ...
      await (0, import_utils28.sleep)(delay);
...
var _Provider = class {
  ...
  async sendTransaction(transactionRequestLike, { estimateTxDependencies = true, enableAssetBurn } = {}) {
    ...
    const encodedTransaction = (0, import_utils29.hexlify)(transactionRequest.toTransactionBytes());
    let abis;
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      ...
      if (hasMissingOutputs && isTransactionTypeScript(transactionRequest)) {
        ...
        missingOutputContractIds.forEach(({ contractId }) => {
          transactionRequest.addContractInputAndOutput(new import_address3.Address(contractId));
  ...
  async estimateMultipleTxDependencies(transactionRequests) {
    ...
    allRequests.forEach((req, index) => {
      if (isTransactionTypeScript(req)) {
        serializedTransactionsMap.set(index, (0, import_utils29.hexlify)(req.toTransactionBytes()));
      }
    });
    ...
    while (transactionsToProcess.length > 0 && attempt < MAX_RETRIES) {
      ...
      for (let i = 0; i < dryRunResults.dryRun.length; i++) {
        ...
        if (hasMissingOutputs && isTransactionTypeScript(request2)) {
          ...
          missingOutputContractIds.forEach(({ contractId }) => {
            request2.addContractInputAndOutput(new import_address3.Address(contractId));
          ...
          serializedTransactionsMap.set(requestIdx, (0, import_utils29.hexlify)(request2.toTransactionBytes()));
  ...
  async dryRunMultipleTransactions(transactionRequests, { utxoValidation, estimateTxDependencies = true } = {}) {
    ...
    const encodedTransactions = transactionRequests.map((tx) => (0, import_utils29.hexlify)(tx.toTransactionBytes()));
    const { dryRun: dryRunStatuses } = await this.operations.dryRun({
      encodedTransactions,
      utxoValidation: utxoValidation || false
    });
  ...
  async estimateTxGasAndFee(params) {
    ...
    if (!(0, import_utils29.isDefined)(gasPrice)) {
  ...
  async simulate(transactionRequestLike, { estimateTxDependencies = true } = {}) {
    ...
    const encodedTransactions = [(0, import_utils29.hexlify)(transactionRequest.toTransactionBytes())];
    const { dryRun: dryRunStatuses } = await this.operations.dryRun({
      encodedTransactions,
      utxoValidation: true
    });
  ...
  async getCoins(owner, assetId, paginationArgs) {
    ...
    const {
      ...
      filter: { owner: ownerAddress.toB256(), assetId: assetId && (0, import_utils29.hexlify)(assetId) }
    });
      ...
      switch (coin.type) {
        case "MessageCoin":
          return {
            ...
            sender: new import_address3.Address(coin.sender),
            recipient: new import_address3.Address(coin.recipient),
            nonce: coin.nonce
          };
        case "Coin":
          return {
  ...
  async getContractBalance(contractId, assetId) {
    const { contractBalance } = await this.operations.getContractBalance({
      ...
      asset: (0, import_utils29.hexlify)(assetId)
    });
  ...
  async getMessages(address, paginationArgs) {
    ...
    const messages = edges.map(({ node }) => ({
      ...
      sender: new import_address3.Address(node.sender),
  ...
  async getMessageProof(transactionId, nonce, commitBlockId, commitBlockHeight) {
    ...
    return {
      ...
      sender: new import_address3.Address(sender),
      recipient: new import_address3.Address(recipient),
      nonce,
  ...
  async produceBlocks(amount, startTime) {
    const { produceBlocks: latestBlockHeight } = await this.operations.produceBlocks({
      ...
      startTimestamp: startTime ? import_utils29.DateTime.fromUnixMilliseconds(startTime).toTai64() : void 0
    });
  ...
  async getMessageByNonce(nonce) {
    ...
    const message = {
      ...
      sender: new import_address3.Address(rawMessage.sender),
...
function getPredicateScriptLoaderInstructions(originalBinary, blobId) {
  ...
  if (configurableSection.length > 0) {
    ...
    return {
      loaderBytecode: (0, import_utils42.concat)([loaderBytecode2, configurableSection]),
...
async function deployScriptOrPredicate({
  ...
  const blobId = getBytecodeId((0, import_utils43.arrayify)(bytecode));
  const configurableOffset = getBytecodeConfigurableOffset((0, import_utils43.arrayify)(bytecode));
  const byteCodeWithoutConfigurableSection = bytecode.slice(0, configurableOffset);
