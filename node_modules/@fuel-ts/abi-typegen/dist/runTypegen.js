...
var Abi = class {
  ...
  constructor(params) {
    ...
    this.capitalizedName = `${(0, import_utils.normalizeString)(abiName[1])}`;
...
function renderFactoryTemplate(params) {
  ...
  const text = renderHbsTemplate({
    ...
    data: {
      ...
      compressedBytecode: (0, import_utils2.compressBytecode)(hexlifiedBinString)
    }
  });
  return text;
}
...
function renderMainTemplate2(params) {
  ...
  const text = renderHbsTemplate({
    ...
    data: {
      ...
      enums,
      abiJsonString,
      compressedBytecode: (0, import_utils3.compressBytecode)(hexlifiedBinString),
      capitalizedName,
      imports,
      configurables,
      commonTypesInUse: commonTypesInUse.join(", ")
...
function renderMainTemplate3(params) {
  ...
  const text = renderHbsTemplate({
    ...
    data: {
      ...
      enums,
      abiJsonString,
      compressedBytecode: (0, import_utils4.compressBytecode)(hexlifiedBinString),
      capitalizedName,
      imports,
      configurables,
      commonTypesInUse: commonTypesInUse.join(", ")
...
var collectBinFilepaths = (params) => {
  ...
  const binFiles = filepaths.map((abiFilepath) => {
    ...
    const bin = {
      path: binFilepath,
      contents: (0, import_utils5.hexlify)((0, import_fs.readFileSync)(binFilepath))
    };
    return bin;
  });
  return binFiles;
...
function runTypegen(params) {
  ...
  const versions = { FUELS: import_versions.versions.FUELS, ...params.versions };
  ...
  const abiFiles = filepaths.map((filepath) => {
    const contents = (0, import_fs3.readFileSync)(filepath, "utf-8");
    const abi = {
      path: filepath,
      contents
    };
  ...
  import_mkdirp.mkdirp.sync(`${output}`);
  abiTypeGen.files.forEach((file) => {
    (0, import_rimraf.rimrafSync)(file.path);
